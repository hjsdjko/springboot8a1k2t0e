	...
	<div class="addEdit-block" :style='{"padding":"30px 0","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="职称" prop="zhicheng">
					<el-input v-model="ruleForm.zhicheng" placeholder="职称" clearable  :readonly="ro.zhicheng"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="职称" prop="zhicheng">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.zixunneirong" label="咨询内容" prop="zixunneirong">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"inherit","background":"none","fontWeight":"500","display":"inline-block"}'>{{ruleForm.zixunneirong}}</span>
				</el-form-item>
		...
		init(id,type) {
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					if(((json.ertongzhanghao!=''&&json.ertongzhanghao) || json.ertongzhanghao==0) && this.$storage.get("role")!="管理员"){
						this.ruleForm.ertongzhanghao = json.ertongzhanghao
						this.ro.ertongzhanghao = true;
					}
					if(((json.ertongxingming!=''&&json.ertongxingming) || json.ertongxingming==0) && this.$storage.get("role")!="管理员"){
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `zixunjilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.zixunjiluCrossAddOrUpdateFlag = false;
											this.parent.search();
