<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","textAlign":"right","display":"block","order":"2"}' >
					...
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="职称" prop="zhicheng">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">职称</label>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('xinliyisheng','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('xinliyisheng','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('xinliyisheng','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"20px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="zhuanyeshanzhang"
						label="专业擅长">
						<template slot-scope="scope">
							{{scope.row.zhuanyeshanzhang}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="lianxidianhua"
					...
					<el-table-column :resizable='true' :sortable='true' prop="zizhiwenjian" label="资质文件">
						<template slot-scope="scope">
							<el-button v-if="scope.row.zizhiwenjian" type="text" size="small" @click="download(scope.row.zizhiwenjian)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<!-- 无 -->
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('xinliyisheng','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('xinliyisheng','在线咨询')" @click="zixunjiluCrossAddOrUpdateHandler(scope.row,'cross','是','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								在线咨询
							</el-button>
							<el-button class="edit" v-if=" isAuth('xinliyisheng','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('xinliyisheng','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<zixunjilu-cross-add-or-update v-if="zixunjiluCrossAddOrUpdateFlag" :parent="this" ref="zixunjiluCrossaddOrUpdate"></zixunjilu-cross-add-or-update>
...
import zixunjiluCrossAddOrUpdate from "../zixunjilu/add-or-update";
	export default {
		data() {
			return {
				zhichengOptions: [],
				...
				zixunjiluCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			zixunjiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.zixunjiluCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','xinliyisheng');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.zixunjiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.zixunjiluCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    init () {
        this.sfshOptions = "是,否,待审核".split(',');
          this.zhichengOptions = "高级,中级,初级".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
		...
		this.$refs["shBatchForm"].validate(valid => {
			if(valid){
				this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
					...
					this.$http({
						url: "xinliyisheng/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
						method: "post",
						data: this.batchIds
					}).then(({ data }) => {
						if (data && data.code === 0) {
	...
	xinliyishengstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'xinliyisheng/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
