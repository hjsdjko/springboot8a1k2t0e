...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>儿童账号：</div>
        <el-input v-model="formSearch.ertongzhanghao" placeholder="儿童账号" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>儿童姓名：</div>
        <el-input v-model="formSearch.ertongxingming" placeholder="儿童姓名" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('liushouertong','新增')" :style='{"cursor":"pointer","border":"1px solid #769589","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#769589","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"35px","height":"35px"}' type="primary" @click="add('/index/liushouertongAdd')"><i v-if="true" :style='{"color":"#769589","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list2 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
			<div :style='{"cursor":"pointer","padding":"0","boxShadow":"0px 4px 10px 0px #769589","margin":"10px 1% 16px","flexWrap":"wrap","background":"#fff","display":"flex","width":"31%","fontSize":"0","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				...
				<div class="item-info" :style='{"width":"100%","padding":"10px 10px 16px","overflow":"hidden","textAlign":"center","display":"inline-block","height":"auto"}'>
					<div :style='{"padding":"0 5px","lineHeight":"20px","fontSize":"14px","color":"#769589","display":"inline-block"}' class="name ">{{item.ertongzhanghao}}</div>
					<div :style='{"padding":"0 5px","lineHeight":"20px","fontSize":"14px","color":"#769589","display":"inline-block"}' class="name ">{{item.ertongxingming}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.ertongzhanghao != '') searchWhere.ertongzhanghao = '%' + this.formSearch.ertongzhanghao + '%';
        if (this.formSearch.ertongxingming != '') searchWhere.ertongxingming = '%' + this.formSearch.ertongxingming + '%';
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`liushouertong/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/liushouertongDetail', query: params});
      },
