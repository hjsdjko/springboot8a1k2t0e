...
<div>
	<div :style='{"border":"1px solid #769589","padding":"10px","margin":"10px auto","borderRadius":"0","display":"flex","width":"86%","justifyContent":"center"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/liushouertong"><a>{{item.name}}</a></el-breadcrumb-item>
	...
	<div class="detail-preview" :style='{"width":"86%","margin":"10px auto","position":"relative","flexDirection":"column","display":"flex"}'>
		<div class="attr" :style='{"padding":"20px 0","position":"relative","order":"1"}'>
			...
			<div class="info" :style='{"padding":"0 0 40px","margin":"0","background":"#F9F8F8","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"0 10% 0 10%","margin":"0 0 10px 0","background":"#F9F8F8","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"right","width":"180px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>家庭情况</div>
					<div  :style='{"padding":"8px 10px 0","boxShadow":"0 1px 0 #999","color":"#000","flex":"1","fontSize":"14px","lineHeight":"24px","height":"auto"}'>{{detail.jiatingqingkuang}}</div>
				</div>
				<div class="btn" :style='{"padding":"0 0 0 180px","margin":"20px 0 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#769589","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('liushouertong','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#769589","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('liushouertong','删除')" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
          ...
          this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
            if (res.data.code == 0) {
              this.detail = res.data.data;
				this.title = this.detail.ertongzhanghao;
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/liushouertongAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此留守儿童？')
			  .then(_ => {
			    this.$http.post('liushouertong/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
