...
<div>
	<div :style='{"border":"1px solid #769589","padding":"10px","margin":"10px auto","borderRadius":"0","display":"flex","width":"86%","justifyContent":"center"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/jiaoyuziyuan"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"padding":"20px 0","position":"relative","order":"1"}'>
			...
			<div class="info" :style='{"padding":"0 0 40px","margin":"0","background":"#F9F8F8","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"0 10% 0 10%","margin":"0 0 10px 0","background":"#F9F8F8","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"right","width":"180px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>资源类型</div>
					<div  :style='{"padding":"8px 10px 0","boxShadow":"0 1px 0 #999","color":"#000","flex":"1","fontSize":"14px","lineHeight":"24px","height":"auto"}'>{{detail.ziyuanleixing}}</div>
				</div>
				<div class="item" :style='{"padding":"0 10% 0 10%","margin":"0 0 10px 0","background":"#F9F8F8","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"right","width":"180px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>资源简介</div>
					<div  :style='{"padding":"8px 10px 0","boxShadow":"0 1px 0 #999","color":"#000","flex":"1","fontSize":"14px","lineHeight":"24px","height":"auto"}'>{{detail.ziyuanjianjie}}</div>
				</div>
				<div class="item" :style='{"padding":"0 10% 0 10%","margin":"0 0 10px 0","background":"#F9F8F8","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="item" :style='{"padding":"0 10% 0 10%","margin":"0 0 10px 0","background":"#F9F8F8","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"right","width":"180px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>资源文件</div>
					<el-button :style='{"padding":"0px 10px","margin":"5px","color":"#fff","borderRadius":"15px","textAlign":"center","background":"#769589","width":"100px","lineHeight":"30px","fontSize":"14px","textDecoration":"none","height":"30px"}' @click="download(detail.ziyuanwenjian)">点击下载</el-button>
				</div>
				<div class="btn" :style='{"padding":"0 0 0 180px","margin":"20px 0 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#769589","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('jiaoyuziyuan','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#769589","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('jiaoyuziyuan','删除')" @click="delClick">删除</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#769589","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('jiaoyuziyuan','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      storeup(type) {
        if (type == 1 && !this.isStoreup) {
          ...
          this.$http.post('storeup/add', this.storeupParams).then(res => {
            if (res.data.code == 0) {
			  ...
			  this.$http.post('jiaoyuziyuan/update', this.detail).then(res => {});
              this.$message({
                type: 'success',
                message: '收藏成功!',
                duration: 1500,
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'jiaoyuziyuan', userid: localStorage.getItem('frontUserid')}}).then(res => {
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/jiaoyuziyuanAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此教育资源？')
			  .then(_ => {
			    this.$http.post('jiaoyuziyuan/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
