...
<div>
	<div :style='{"border":"1px solid #769589","padding":"10px","margin":"10px auto","borderRadius":"0","display":"flex","width":"86%","justifyContent":"center"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/zhiyuanzhe"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"padding":"20px 0","position":"relative","order":"1"}'>
			...
			<div class="info" :style='{"padding":"0 0 40px","margin":"0","background":"#F9F8F8","flex":"1"}'>
				...
				<div class="btn" :style='{"padding":"0 0 0 180px","margin":"20px 0 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#769589","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('zhiyuanzhe','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#769589","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('zhiyuanzhe','删除')" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
          ...
          this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
            if (res.data.code == 0) {
              this.detail = res.data.data;
				this.title = this.detail.zhiyuanzhexingming;
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/zhiyuanzheAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此志愿者？')
			  .then(_ => {
			    this.$http.post('zhiyuanzhe/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
